...
if(isset($_POST['Test'])){
	...
	if($Test->save()){	Yii::app()->user->setFlash('success','Test successfully Created'); $Test->unsetAttributes(); } //end model save	
}//end post
else{	  }
...
$this->render('admin'.$model->panel_id, array('model'=>$model, 'Test'=>$Test,));
	...
	if( Yii::app()->request->isAjaxRequest )
		  {
			...
			exit;
          	...
          	Yii::app()->user->setFlash('success','Test successfully created.');	
		...
		if( Yii::app()->request->isAjaxRequest ){
			...
			Yii::app()->clientScript->scriptMap['jquery.js'] = false;
	 		echo CJSON::encode( array(
			  ...
			  'content' => 'Test successfully Updated',
			));
		exit;
		}else{
		Yii::app()->user->setFlash('success','Test successfully Updated');	
		$this->redirect(array('/Test/admin/'.$model->panel_id));
		}
	}//end if model save
}//end if POST
