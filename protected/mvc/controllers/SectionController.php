	...
	if( Yii::app()->request->isAjaxRequest )
		  {
			...
			exit;
          	...
          	Yii::app()->user->setFlash('success','Section successfully created.');	
		...
		if( Yii::app()->request->isAjaxRequest ){
			...
			Yii::app()->clientScript->scriptMap['jquery.js'] = false;
	 		echo CJSON::encode( array(
			  ...
			  'content' => 'Section successfully Updated',
			));
		exit;
		}else{
		Yii::app()->user->setFlash('success','Section successfully Updated');	
		$this->redirect(array('section/admin'));
		}
	}//end if section save
}//end if POST
				...
				if( Yii::app()->request->isAjaxRequest ){
					...
					$this->render( 'update', array( 'section' => $section) );
				  }
