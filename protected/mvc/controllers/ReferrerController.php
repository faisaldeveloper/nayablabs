...
{
$model->attributes=$_POST['Referrer'];
	...
	if( Yii::app()->request->isAjaxRequest )
		  {
			...
			exit;
          	...
          	Yii::app()->user->setFlash('success','Referrer successfully created.');	
		...
		if( Yii::app()->request->isAjaxRequest ){
			...
			Yii::app()->clientScript->scriptMap['jquery.js'] = false;
	 		echo CJSON::encode( array(
			  ...
			  'content' => 'Referrer successfully Updated',
			));
		exit;
		}else{
		Yii::app()->user->setFlash('success','Referrer successfully Updated');	
		$this->redirect(array('admin'));
		}
	}//end if model save
}//end if POST
...
$this->actionAdmin();
exit;
$dataProvider=new CActiveDataProvider('Referrer');
...
{
$model=new Referrer('search');
$model->unsetAttributes();  // clear any default values
if(isset($_GET['Referrer']))
$model->attributes=$_GET['Referrer'];
...
{
$model=Referrer::model()->findByPk($id);
if($model===null)
...
{
if(isset($_POST['ajax']) && $_POST['ajax']==='referrer-form')
{
