...
?>
<?php
if(isset($_GET['id'])&& $_GET['id']>0 && $_GET['id']<=20){
$weekno = $_GET['id'];	
}else{
$weekno = Yii::app()->user->report_lastyear;
}

$MY= "20".str_pad($weekno,2,'0',STR_PAD_LEFT)."";
?>
<h1>Yearly Sales Report(<?php echo $MY;?>)</h1>
<style>
.totalsclass{
	text-align:right;
}
...
//=================================================================
$previous_disabled = '';
$previous_link=Yii::app()->baseUrl.'/reports/yearly/'.($weekno-1==0?'':$weekno-1);
if($weekno<=Yii::app()->user->report_firstyear){
$previous_disabled = 'disabled';
$previous_link='javascript:void(0)';
}

$next_disabled = '';
$next_link =Yii::app()->baseUrl.'/reports/yearly/'.($weekno+1==0?'':$weekno+1);
if($weekno>=Yii::app()->user->report_lastyear){
...
$resultset  =  Yii::app()->db->createCommand("Select date_of_tableorder,SUBSTRING_INDEX(`date_of_tableorder`, '-', 2) as `mycol` FROM `tableorder` WHERE date_of_tableorder like '".$MY."-%'")->queryAll();

$dates = array();
foreach($resultset as $k=>$row){
$dates[$row['mycol']]['label']=$row['mycol'];
}
...
'columns'=>array(
		...
		array('name'=>'date_of_tableorder','value'=>'date("M, Y",strtotime($data->date_of_tableorder))',
		'header'=>'Month','htmlOptions'=>array('id'=>"-".$data->date_of_tableorder."-"),),
		//'comments',
		//'cover',
		//'gst',
...
'total' => array('label'=>'<span style="font-size:1.5em">Yearly Sale </span>', 'class'=>'TbSumOperation')
)
),
   ...
   //add item to array
	...
	google.load("visualization", "1", {packages:["corechart"]});
      ...
      function drawChart() {
        ...
        var options = {
          title: 'Yearly Sales Report Graph (<?php echo $MY;?>)',
   ...
   //add item to array
	...
	$('#mychart_div').css('height',rows+'px');
        $('#mychart_div').highcharts({
            ...
            title: {
                text: 'Yearly Sales Report Graph (<?php echo $MY;?>)'
            },
            subtitle: {
