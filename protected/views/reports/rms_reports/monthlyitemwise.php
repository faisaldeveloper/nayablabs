...
?>
<h1>Monthly Item Wise Report(<?php echo date('M, Y',strtotime($MY));?>)</h1>
<style>
.totalsclass{
	text-align:right;
}
...
$previous_link=Yii::app()->baseUrl.'/reports/monthlyitemwise/'.$linkparam_pre;
...
$next_link =Yii::app()->baseUrl.'/reports/monthlyitemwise/'.$linkparam_next;
...
$resultset  =  Yii::app()->db->createCommand("Select * FROM `tableorder` WHERE date_of_tableorder like '20".$yearno."-".str_pad($weekno,2,'0',STR_PAD_LEFT)."-%'")->queryAll();
   ...
   cols++;
	...
	//griddates = [['Year', 'Sales', 'Profit'],['2004',  82494.5,	8494.5],['2005',  9259,      9240]];
      ...
      function drawChart() {
        ...
        var options = {
           title: 'Monthly Item Wise Report Graph(<?php echo date('M, Y',strtotime($MY));?>)',
   ...
   //add item to array
	...
	$('#mychart_div').css('height',rows+'px');
        $('#mychart_div').highcharts({
            ...
            title: {
                text: 'Monthly Item Wise Report Graph(<?php echo date('M, Y',strtotime($MY));?>)'
            },
            subtitle: {
